...
async def start(client, message):
    ...
    if len(message.command) != 2:
        ...
        reply_markup = InlineKeyboardMarkup(buttons)
        m=await message.reply_sticker("CAACAgUAAxkBAAIRQmdZTp1gPsdPebVQDS_cc0YKkbetAAL-BQACeTdpV5IcBm2dC-scHgQ") 
    ...
    if data.startswith("sendfiles"):
        ...
        for file in files:
            ...
            if STREAM_MODE == True:
                button.append([InlineKeyboardButton('🚀 Fᴀsᴛ Dᴏᴡɴʟᴏᴀᴅ / Wᴀᴛᴄʜ Oɴʟɪɴᴇ 🖥️', callback_data=f'generate_stream_link:{file_id}')])
            msg = await client.send_cached_media(
                chat_id=message.from_user.id,
                file_id=file_id,
                caption=f_caption,
...
async def delete(bot, message):
    ...
    if result.deleted_count:
        ...
        caption = re.sub(r"(_|\-|\.|\+)", " ", str(media.file_name))
        unwanted_chars = ['[', ']', '(', ')']
