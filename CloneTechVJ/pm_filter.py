    ...
    btn = [
        [
            InlineKeyboardButton(
                text=f"[{get_size(filevj['file_size'])}] {' '.join(filter(lambda x: not x.startswith('[') and not x.startswith('@') and not x.startswith('www.'), filevj['caption'].split()))}", callback_data=f"file#{filevj['file_id']}"
            ),
        ]
        for filevj in files
    ]
...
async def filter_qualities_cb_handler(client: Client, query: CallbackQuery):
    ...
    btn = [
        [
            InlineKeyboardButton(
                text=f"⚜️[{get_size(filevj['file_size'])}] {' '.join(filter(lambda x: not x.startswith('[') and not x.startswith('@') and not x.startswith('www.'), filevj['caption'].split()))}", callback_data=f"file#{filevj['file_id']}"
            ),
        ]
        for filevj in files
    ] 
